...
export default function AdminDashboard() {
  ...
  return (
    <DashboardLayout title="School Administration">
        ...
        {TABS.map((t) => (
          ...
          >
            ...
            {t === "Correction Requests" &&
              overview?.pendingCorrections > 0 && (
                <span className="badge ml-1.5 bg-accent-400/20 text-accent-600">
                  {overview.pendingCorrections}
                </span>
              )}
          </button>
        ))}
      ...
      <div style={{ display: tab === "Admins" ? "" : "none" }}>
        <UserManager
          role="ADMIN"
          schoolConfig={schoolConfig}
          onChanged={loadOverview}
...
function StudentRecordsManager({ schoolConfig, onChanged }) {
  ...
  const loadAll = () =>
    api.get("/admin/students").then((res) => setAllStudents(res.data.students));
  useEffect(() => {
    loadAll();
  }, []);
  ...
  useEffect(() => {
    ...
    api
      .get("/admin/students", {
        params: { class: selectedClass, section: selectedSection || undefined },
      })
  ...
  const create = async (e) => {
    ...
    try {
      ...
      setStatus(
        err.response?.data?.message || "Could not create student record.",
      );
    }
  };
  ...
  const removeStudent = async (student) => {
    try {
      ...
      setStatus(
        err.response?.data?.message || "Could not remove this student.",
      );
    }
  };
    ...
    <div className="space-y-6">
      <div className="card">
        ...
        <p className="mt-1 text-sm text-ink-soft">
          Create the official record before a student registers — no password
          needed here. The student sets their own password when they verify
          against this record.
        </p>
        <form
          onSubmit={create}
        ...
        >
          ...
          />
          <button className="btn-primary sm:col-span-2 lg:col-span-5">
            Add Student Record
          </button>
        </form>
      ...
      {search.trim() ? (
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              {(schoolConfig.sectionsByClass?.[selectedClass] || []).map(
                (s) => (
          ...
          {selectedSection ? (
            <StudentTable
              ...
              onRemove={removeStudent}
              emptyText={`No students in Class ${selectedClass} Section ${selectedSection}.`}
            />
          ) : (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {classesToShow.map((cls) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-ink-faint">
                {classCounts[cls] || 0} student record
                {classCounts[cls] === 1 ? "" : "s"}
              </p>
          ...
          {classesToShow.length === 0 && (
            <p className="text-sm text-ink-faint">No classes configured yet.</p>
          )}
        </div>
      )}
    </div>
    ...
    <div className="overflow-x-auto rounded-lg border border-brand-100 bg-canvas-card">
      <table className="min-w-full divide-y divide-brand-100 text-sm">
        <thead className="bg-canvas-sunk">
          <tr>
            <th className="px-4 py-2 text-left font-medium text-ink-faint">
              Student ID
            </th>
            <th className="px-4 py-2 text-left font-medium text-ink-faint">
              Name
            ...
            <tr key={s._id}>
              ...
              <td className="px-4 py-2">
                {s.isRegistered ? (
                  ...
                  <span className="badge bg-canvas-sunk text-ink-faint">
                    Not registered
                  </span>
                )}
              </td>
...
function UserManager({ role, schoolConfig, onChanged }) {
  ...
  const resetPassword = async (id) => {
    const newPassword = window.prompt(`Enter a new password for this ${roleLabel.toLowerCase()} (minimum 8 characters):`);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        />
        <button className="btn-primary">Add {roleLabel}</button>
      </form>
                ...
                <td className="px-4 py-2 text-ink-faint">
                  {(u.teacherAssignments || []).length === 0
                    ...
                    : u.teacherAssignments
                        .map(
                          (a) =>
                            `${a.subject} ${a.class}${a.section ? `-${a.section}` : ""}`,
                        )
                        .join(", ")}
                </td>
                <td className="px-4 py-2">
    ...
    >
      <div className="card w-full max-w-lg">
        <h3 className="text-lg font-semibold text-ink">
          Assign Classes — {teacher.fullName}
        </h3>
        <p className="mt-1 text-sm text-ink-soft">
          This teacher only gets access to what's listed below. Leave section
          empty for the whole class.
        </p>
        ...
        <div className="mt-4 space-y-2">
          {assignments.map((a, idx) => (
            <div
              key={idx}
              className="flex items-center justify-between rounded-md bg-canvas-sunk px-3 py-2 text-sm"
            >
              <span>
                {a.subject} — Class {a.class}
                {a.section ? ` Section ${a.section}` : " (whole class)"}
              </span>
              <button
          ...
          {assignments.length === 0 && (
            <p className="text-sm text-ink-faint">No classes assigned yet.</p>
          )}
        </div>
        ...
        {requests.map((r) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span
                className={`badge ${r.status === "pending" ? "bg-accent-400/20 text-accent-600" : r.status === "rejected" ? "bg-red-50 text-red-700" : "bg-sage-100 text-sage-700"}`}
              >
                {r.status.replace("_", " ")}
              </span>
            </div>
...
function CorrectionRequestDetail({ request, onClose, onResolved }) {
  ...
  const resolve = async (newStatus, applyUpdate) => {
    try {
      ...
      setStatus(
        err.response?.data?.message || "Could not update this request.",
      );
    }
  };
  ...
  return (
    ...
    >
      <div className="card w-full max-w-lg">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold text-ink">
            Correction Request {request.requestCode}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="mt-3 rounded-lg bg-canvas-sunk p-3 text-sm">
          <p className="font-medium text-ink">
            {ISSUE_LABELS[request.issueType]}
          </p>
        ...
        {request.student && (
          <div className="mt-4 grid grid-cols-2 gap-3">
            <input
              className="input-field"
              placeholder="Name"
              value={recordUpdate.fullName}
              onChange={(e) =>
                setRecordUpdate({ ...recordUpdate, fullName: e.target.value })
              }
            />
            <input
              className="input-field"
              placeholder="Class"
              value={recordUpdate.class}
              onChange={(e) =>
                setRecordUpdate({ ...recordUpdate, class: e.target.value })
              }
            />
            <input
              className="input-field"
              placeholder="Section"
              value={recordUpdate.section}
              onChange={(e) =>
                setRecordUpdate({ ...recordUpdate, section: e.target.value })
              }
            />
            <input
              className="input-field"
              placeholder="Roll No."
              value={recordUpdate.rollNumber}
              onChange={(e) =>
                setRecordUpdate({ ...recordUpdate, rollNumber: e.target.value })
              }
            />
          </div>
        )}
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <button
            onClick={() => resolve("rejected", false)}
            className="btn-danger"
          >
            Reject Request
          </button>
          <button
            onClick={() => resolve("under_review", false)}
            className="btn-secondary"
          >
            Mark Under Review
          </button>
...
function SchoolSettings({ schoolConfig, onChanged }) {
  ...
  const regenerateCode = async () => {
    ...
    try {
      ...
      setCodeStatus(
        err.response?.data?.message || "Could not regenerate the school code.",
      );
    } finally {
  ...
  const toggleClass = async (cls) => {
    ...
    try {
      ...
      setClassStatus(
        `Class ${cls} ${nextClasses.includes(cls) ? "enabled" : "removed"}.`,
      );
      onChanged();
    } catch (err) {
      setClassStatus(
        err.response?.data?.message || "Could not update classes.",
      );
    }
  };
  ...
  const saveSections = async () => {
    try {
      await api.put(`/admin/school/classes/${expandedClass}/sections`, {
      ...
      setClassStatus(
        err.response?.data?.message || "Could not update sections.",
      );
    }
  };
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="card space-y-3">
        ...
        <p className="text-sm text-ink-faint">
          Regenerating replaces the code everyone uses to log in and register.
          It does not delete or change any student, teacher, or school record —
      ...
      <div className="card">
        ...
        <p className="mt-1 text-sm text-ink-soft">
          Choose which of Class 1–12 this school uses, then configure which
          sections exist for each — not every class needs the same sections.
        </p>
        {classStatus && (
          <p className="mt-2 text-sm text-sage-700">{classStatus}</p>
        )}
              ...
              <div key={cls} className="rounded-md border border-brand-100 p-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-ink">
                    Class {cls}:{" "}
                    {(schoolConfig.sectionsByClass?.[cls] || []).join(", ") ||
...
function AdminAccountSettings() {
  ...
  const changeId = async (e) => {
    ...
    try {
      ...
      setIdStatus(
        "Admin ID updated. Use the new ID for your next login — this session keeps working.",
      );
  ...
  const generateRecoveryCode = async (e) => {
    ...
    try {
      const res = await api.post("/admin/account/recovery-code", {
      ...
      setRecoveryStatus(
        err.response?.data?.message || "Could not generate a recovery code.",
      );
    }
  };
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="card">
        ...
        <dl className="mt-3 space-y-1.5 text-sm">
          <div className="flex justify-between">
            <dt className="text-ink-soft">Admin Name</dt>
            <dd className="text-ink">{account.adminName}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-ink-soft">Admin ID</dt>
            <dd className="text-ink">{account.adminId}</dd>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <dd className="text-ink">{account.school}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-ink-soft">School Code</dt>
            <dd className="text-ink">{account.schoolCode}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-ink-soft">Password</dt>
            <dd className="text-ink">Set</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-ink-soft">Recovery Code</dt>
            <dd className="text-ink">
              {account.recoveryCode.configured
                ? "Configured"
                : "Not configured"}
            </dd>
          </div>
      ...
      <form onSubmit={changeId} className="card space-y-4">
        ...
        <p className="text-sm text-ink-soft">
          Current Admin ID: <strong>{account.adminId}</strong>
        </p>
        {idStatus && <p className="text-sm text-sage-700">{idStatus}</p>}
        <input
          className="input-field"
          placeholder="New Admin ID"
          value={idForm.newAdminId}
          onChange={(e) => setIdForm({ ...idForm, newAdminId: e.target.value })}
          required
        />
        <input
          type="password"
          ...
          onChange={(e) =>
            setIdForm({ ...idForm, currentPassword: e.target.value })
          }
          required
        />
      ...
      <form onSubmit={generateRecoveryCode} className="card space-y-4">
        ...
        <p className="text-sm text-ink-soft">
          Used to reset your password if you ever forget it, without needing
          internet or email. Generating a new code immediately invalidates the
          old one.
        </p>
        {recoveryStatus && (
          <p className="text-sm text-red-600">{recoveryStatus}</p>
        )}
        {recoveryResult && (
